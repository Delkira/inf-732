...
describe('NotasController (e2e)', () => {
    ...
    describe('/notas/:title (GET)', () => {
        ...
        it('debería devolver un error 404 si no se encuentran notas con el título dado', async () => {
        const response = await request(app.getHttpServer())
            .get('/notas/titulo/inexistente')
            .expect(404);

        expect(response.body.message).toContain('nota con el título inexistente no encontrada');
        });
    })
});